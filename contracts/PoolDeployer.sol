...
contract PoolDeployer {
    ...
    {
        return address(
            new PoolConfigurator{salt: keccak256(abi.encode(marketId_))}(IPoolAddressesProvider(poolAddressesProvider_))
        );
    }
    ...
    {
        return address(
            new LoanManager{salt: keccak256(abi.encode(marketId_))}(IPoolAddressesProvider(poolAddressesProvider_))
        );
    }
    ...
    {
        return address(
            new WithdrawalManager{salt: keccak256(abi.encode(marketId_))}(IPoolAddressesProvider(poolAddressesProvider_))
        );
    }
}
