...
contract PoolFactory is PoolDeployer, Adminable {
    ...
    function createPool(
        ...
        external
        returns (address poolAddressesProvider_)
    {
        // Caching
        ...
        poolAddressesProvider_ =
            address(new PoolAddressesProvider{salt: keccak256(abi.encode(marketId_))}(marketId_, admin_));
        ...
        // Set implementations
        IPoolAddressesProvider(poolAddressesProvider_).setPoolConfiguratorImpl(
            poolConfigurator_, asset_, name_, symbol_
        );
