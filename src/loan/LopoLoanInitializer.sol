...
import { ILopoLoanInitializer } from "./interfaces/ILopoLoanInitializer.sol";
...
contract LopoLoanInitializer is ILopoLoanInitializer, LopoLoanStorage {
        ...
        internal
    {
        ...
        require(amounts_[1] > uint256(0), "LPI:I:INVALID_PRINCIPAL");

        // Ending principal needs to be less than or equal to principal requested.
        require(amounts_[2] <= amounts_[1], "LPI:I:INVALID_ENDING_PRINCIPAL");

        // Payment interval and payments remaining need to be non-zero.
        require(termDetails_[0] >= 12 hours, "LPI:I:INVALID_GRACE_PERIOD");
        require(termDetails_[1] > 0,         "LPI:I:INVALID_PAYMENT_INTERVAL");
        require(termDetails_[2] > 0,         "LPI:I:INVALID_PAYMENTS_REMAINING");

        uint256 maxOriginationFee_ = amounts_[1] * 0.025e6 / 1e6;  // 2.5% of principal

        require(fees_[0] <= maxOriginationFee_, "LPI:I:INVALID_ORIGINATION_FEE");

        IGlobalsLike globals_ = IGlobalsLike(ILopoProxyFactoryLike(msg.sender).lopoGlobals());

        require((_borrower = borrower_) != address(0),  "LPI:I:ZERO_BORROWER");
        require(globals_.isBorrower(borrower_),         "LPI:I:INVALID_BORROWER");
        require(globals_.isPoolAsset(assets_[1]),       "LPI:I:INVALID_FUNDS_ASSET");
        require(globals_.isCollateralAsset(assets_[0]), "LPI:I:INVALID_COLLATERAL_ASSET");

        require((_lender = lender_) != address(0), "LPI:I:ZERO_LENDER");

        address loanManagerFactory_ = ILenderLike(lender_).factory();

        require(ILenderLike(lender_).fundsAsset() == assets_[1],                       "LPI:I:DIFFERENT_FUNDS_ASSET");
        require(globals_.isInstanceOf("FT_LOAN_MANAGER_FACTORY", loanManagerFactory_), "LPI:I:INVALID_FACTORY");
        require(ILopoProxyFactoryLike(loanManagerFactory_).isInstance(lender_),       "LPI:I:INVALID_INSTANCE");

        require((_feeManager = feeManager_) != address(0),         "LPI:I:INVALID_MANAGER");
        require(globals_.isInstanceOf("FEE_MANAGER", feeManager_), "LPI:I:INVALID_FEE_MANAGER");
        ...
        ILopoLoanFeeManager(feeManager_).updateDelegateFeeTerms(fees_[0], fees_[1]);
    }
