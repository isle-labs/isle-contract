...
import { LopoProxiedInternals } from "lopo-proxy-factory/LopoProxiedInternals.sol";

import { ILopoLoan }           from "./interfaces/ILopoLoan.sol";
import { ILopoLoanFeeManager } from "./interfaces/ILopoLoanFeeManager.sol";

import { IGlobalsLike, ILenderLike, ILopoProxyFactoryLike } from "./interfaces/Interfaces.sol";
...
contract LopoLoan is ILopoLoan, LopoProxiedInternals, LopoLoanStorage {
    ...
    modifier limitDrawableUse() {
        ...
        require(_drawableFunds >= drawableFundsBeforePayment, "LP:CANNOT_USE_DRAWABLE");
    }
    ...
    function migrate(address migrator_, bytes calldata arguments_) external override whenNotPaused {
        require(msg.sender == _factory(),        "LP:M:NOT_FACTORY");
        require(_migrate(migrator_, arguments_), "LP:M:FAILED");
    }

    function setImplementation(address newImplementation_) external override whenNotPaused {
        require(msg.sender == _factory(),               "LP:SI:NOT_FACTORY");
        require(_setImplementation(newImplementation_), "LP:SI:FAILED");
    }

    function upgrade(uint256 toVersion_, bytes calldata arguments_) external override whenNotPaused {
        require(msg.sender == IGlobalsLike(globals()).securityAdmin(), "LP:U:NO_AUTH");

        emit Upgraded(toVersion_, arguments_);

        ILopoProxyFactory(_factory()).upgradeInstance(toVersion_, arguments_);
    }
    ...
    function acceptBorrower() external override whenNotPaused {
        require(msg.sender == _pendingBorrower, "LP:AB:NOT_PENDING_BORROWER");
        ...
        external override whenNotPaused limitDrawableUse returns (uint256 principal_, uint256 interest_, uint256 fees_)
    {
        ...
        require(
            ...
            "LP:CL:TRANSFER_FROM_FAILED"
        );

        uint256 paymentDueDate_ = _nextPaymentDueDate;

        require(block.timestamp <= paymentDueDate_, "LP:CL:PAYMENT_IS_LATE");
        ...
        require(ERC20Helper.transfer(_fundsAsset, _lender, principalAndInterest_), "LP:MP:TRANSFER_FAILED");
    ...
    function drawdownFunds(uint256 amount_, address destination_) external override whenNotPaused onlyBorrower returns (uint256 collateralPosted_) {
        ...
        require(ERC20Helper.transfer(_fundsAsset, destination_, amount_), "LP:DF:TRANSFER_FAILED");
        require(_isCollateralMaintained(),                                "LP:DF:INSUFFICIENT_COLLATERAL");
    }
        ...
        external override whenNotPaused limitDrawableUse returns (uint256 principal_, uint256 interest_, uint256 fees_)
    {
        ...
        require(_isCollateralMaintained(), "LP:MP:INSUFFICIENT_COLLATERAL");
    }
    ...
    {
        require(deadline_ >= block.timestamp,                                       "LP:PNT:INVALID_DEADLINE");
        require(IGlobalsLike(globals()).isInstanceOf("FT_REFINANCER", refinancer_), "LP:PNT:INVALID_REFINANCER");
        require(calls_.length > uint256(0),                                         "LP:PNT:EMPTY_CALLS");
    ...
    function removeCollateral(uint256 amount_, address destination_) external override whenNotPaused onlyBorrower {
        ...
        require(ERC20Helper.transfer(_collateralAsset, destination_, amount_), "LP:RC:TRANSFER_FAILED");
        require(_isCollateralMaintained(),                                     "LP:RC:INSUFFICIENT_COLLATERAL");
    }
    ...
    function setPendingBorrower(address pendingBorrower_) external override whenNotPaused onlyBorrower {
        require(IGlobalsLike(globals()).isBorrower(pendingBorrower_), "LP:SPB:INVALID_BORROWER");
    ...
    function acceptLender() external override whenNotPaused {
        require(msg.sender == _pendingLender, "LP:AL:NOT_PENDING_LENDER");
        ...
        external override whenNotPaused onlyLender returns (bytes32 refinanceCommitment_)
    {
        ...
        require(
            _refinanceCommitment == (refinanceCommitment_ = _getRefinanceCommitment(refinancer_, deadline_, calls_)),
            "LP:ANT:COMMITMENT_MISMATCH"
        );

        require(refinancer_.code.length != uint256(0), "LP:ANT:INVALID_REFINANCER");

        require(block.timestamp <= deadline_, "LP:ANT:EXPIRED_COMMITMENT");
        ...
        ILopoLoanFeeManager feeManager_ = ILopoLoanFeeManager(_feeManager);
        ...
        for (uint256 i_; i_ < calls_.length; ++i_) {
            ...
            require(success_, "LP:ANT:FAILED");
        }
        ...
        require(_isCollateralMaintained(),                       "LP:ANT:INSUFFICIENT_COLLATERAL");
        require(getUnaccountedAmount(fundsAsset_) == uint256(0), "LP:ANT:UNEXPECTED_FUNDS");
    }
    ...
    function fundLoan() external override whenNotPaused onlyLender returns (uint256 fundsLent_) {
        ...
        require((_nextPaymentDueDate == uint256(0)) && (_paymentsRemaining != uint256(0)), "LP:FL:LOAN_ACTIVE");

        address fundsAsset_         = _fundsAsset;
        uint256 paymentInterval_    = _paymentInterval;
        uint256 principalRequested_ = _principalRequested;

        require(ERC20Helper.approve(fundsAsset_, _feeManager, type(uint256).max), "LP:FL:APPROVE_FAIL");

        // Saves the platform service fee rate for future payments.
        ILopoLoanFeeManager(_feeManager).updatePlatformServiceFee(principalRequested_, paymentInterval_);

        uint256 originationFees_ = ILopoLoanFeeManager(_feeManager).payOriginationFees(fundsAsset_, principalRequested_);

        _drawableFunds += (principalRequested_ - originationFees_);

        require(getUnaccountedAmount(fundsAsset_) == uint256(0), "LP:FL:UNEXPECTED_FUNDS");
    ...
    function removeLoanImpairment() external override whenNotPaused onlyLender {
        ...
        require(originalNextPaymentDueDate_ != 0,               "LP:RLI:NOT_IMPAIRED");
        require(block.timestamp <= originalNextPaymentDueDate_, "LP:RLI:PAST_DATE");
    ...
    {
        require((msg.sender == _borrower) || (msg.sender == _lender), "LP:RNT:NO_AUTH");

        require(
            _refinanceCommitment == (refinanceCommitment_ = _getRefinanceCommitment(refinancer_, deadline_, calls_)),
            "LP:RNT:COMMITMENT_MISMATCH"
        );
    ...
    function skim(address token_, address destination_) external override whenNotPaused returns (uint256 skimmed_) {
        ...
        require(ERC20Helper.transfer(token_, destination_, skimmed_), "LP:S:TRANSFER_FAILED");
    }
    ...
    function getClosingPaymentBreakdown() public view override returns (uint256 principal_, uint256 interest_, uint256 fees_) {
        ...
        ) = ILopoLoanFeeManager(_feeManager).getServiceFeeBreakdown(address(this), _paymentsRemaining);
    ...
    function globals() public view override returns (address globals_) {
        globals_ = ILopoProxyFactoryLike(_factory()).lopoGlobals();
    }
    ...
    /// @dev Returns principal and interest portions of a payment installment, given generic, stateless loan parameters.
        ...
        )
    {
        ...
        (
            ...
            uint256 platformServiceFee_,
            uint256 platformRefinanceFee_
        ) = ILopoLoanFeeManager(_feeManager).getServiceFeeBreakdown(address(this), 1);
    ...
    function _handleServiceFeePayment(uint256 numberOfPayments_) internal returns (uint256 fees_) {
        ...
        ILopoLoanFeeManager(_feeManager).payServiceFees(_fundsAsset, numberOfPayments_);
    ...
    function _revertIfNotBorrower() internal view {
        require(msg.sender == _borrower, "LP:NOT_BORROWER");
    }

    function _revertIfNotLender() internal view {
        require(msg.sender == _lender, "LP:NOT_LENDER");
    }
