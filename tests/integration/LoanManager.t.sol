...
//         accruedInterest_ = loanManager.accruedInterest();
...
//         vm.warp(block.timestamp + 100 days);
//         assetsUnderManagement_ = loanManager.assetsUnderManagement();
//         assertEq(assetsUnderManagement_, 200_000e6 + newRateFirst * 30 days / 1e27 + newRateSecond * 100 days /
// 1e27);
//     }
...
//         (principal_, interest_) = loanManager.getLoanPaymentDetailedBreakdown(1);
...
//         uint256 latePeriodicInterestRate = uint256(0.32e6) * (1e18 / 1e6) * fullDaysLate_ / 365 days; // e6 * e18 /
// e6 =
//             // e18
...
//         (principal_, interest_) = loanManager.getLoanPaymentBreakdown(1);
...
//         // before balance
//         uint256 poolBalanceBefore = IERC20(address(usdc)).balanceOf(address(pool));
//         uint256 poolAdminBalanceBefore = IERC20(address(usdc)).balanceOf(users.poolAdmin);
//         uint256 protocolVaultBalanceBefore = IERC20(address(usdc)).balanceOf(ILopoGlobals(lopoGlobals).lopoVault());
...
//         // after balance
//         uint256 poolBalanceAfter = IERC20(address(usdc)).balanceOf(address(pool));
//         uint256 poolAdminBalanceAfter = IERC20(address(usdc)).balanceOf(users.poolAdmin);
//         uint256 protocolVaultBalanceAfter = IERC20(address(usdc)).balanceOf(ILopoGlobals(lopoGlobals).lopoVault());
